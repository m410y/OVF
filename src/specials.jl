...
erf(x::Real) = begin
    ...
    int = 0.0
    if abs(x) < 1.0
        int_range = range(0, abs(x), length = 2 + Int(abs(x) ÷ 2e-4))
        int += simpson_integral(t -> exp(-t^2), int_range)
    else
        int += simpson_integral(t -> exp(-t^2), 0:4e-4:1)
        n_bins = Int(abs(x) ÷ 4e-4)
...
jm(x::Real, m::Integer) =
    1 / pi * simpson_integral(t -> cos(m * t - x * sin(t)), range(0, pi, 64))
